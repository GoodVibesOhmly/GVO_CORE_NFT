...
describe("itemv2 projections ERC721Wrapper", () => {
  ...
  async function test652(){
    ...
    var token3and4 = [
      "128749172",
      "128747931",]
    var receivers3and4 = [[accounts[2], accounts[3], accounts[4]],
                      [accounts[0], accounts[3], accounts[4]]];
    var amounts3and4 = [["300000000000000000", "300000000000000000", "400000000000000000"],["300000000000000000", "300000000000000000", "400000000000000000"]]


    var token5 = [
      "21418451"]
    var receivers5 = [[accounts[1], accounts[3]]];
    var amounts5 = [["300000000000000000", "700000000000000000"]]
    var amounts5catchCallMin = [["30000000000000000", "700000000000000000"]]
    var amounts5catchCallMax = [["3000000000000000000", "700000000000000000"]]
    ...
    var receivers6and7 = [[accounts[3]], [accounts[4]]];
    var amounts6and7 = [["1000000000000000000"], ["1000000000000000000"]]
    ...
    var burn = await Promise.all(
      itemsList.slice(3, 4).map(async (item, index) => {
        return web3.eth.abi.encodeParameters(
          ["address", "uint256", "address", "bytes", "bool", "bool"],
          [knowledgeBase.ensTokenAddress, item.tokenId, accounts[index], "0x", false, false]
        );
      })
    );
  ...
  it("#657 Testing some different unwrap and rewrap scenarios with different balances using the Interoperable burn operation: scenario4", async () => {
    ...
    */
    ...
    await erc20Contract.methods
      ...
      await catchCall(
        wrapper.methods
          .setApprovalForAll(accounts[2], true)
          .send(blockchainConnection.getSendingOptions({ from: accounts[3] })),
        "call directly the setApprovalForAll on the main Interface 0x915a22a152654714fceca3f4704fcf6bd314624c"
      );

    await mainInterface.methods.setApprovalForAll(accounts[2], true).send(blockchainConnection.getSendingOptions({from : accounts[3]}));
